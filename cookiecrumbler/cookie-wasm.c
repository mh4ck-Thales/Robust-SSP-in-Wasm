...
#include <stdlib.h>
#include <stdio.h>
// Not using signals as it is not useful for Wasm
// #include <signal.h>
#include <fcntl.h>
...
{
// None of the options here is triggering for Wasm target
// So we need to set the target manually
// musl libc does use the __stack_chk_guard symbol so we'll use that
...
#elif defined(__FreeBSD__) || defined(__linux__) || defined(__APPLE__) || defined(__wasm32__) && defined(__wasi__)
...
/* this part of get_writeable_range is not needed in Wasm, and uses incomptable signals
#if defined(_WIN32) || defined(_WIN64)
...
void * measure(void *type)
{
	...
	printf("%-13s 0x%016lx 0x%016lx 0x%016lx 0x%016lx 0x%016lx 0x%016lx\n",
		(char *) type,
...
// #endif
	...
	printf("%13s %18s %18s %18s %18s %18s %18s\n",
		"type",
		"local_addr",
		"cookie_addr",
		"cookie_val",
